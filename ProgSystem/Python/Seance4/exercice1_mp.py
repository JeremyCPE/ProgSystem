...
def ecrire_un_message(nom_tache, mess) :
...
def tache_capteur_pression(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou) :  
    while True :
        time.sleep(1)
        verrou.acquire()
        # La pression
...
def tache_capteur_temperature(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou) :  
    while True :
        time.sleep(1)
        verrou.acquire()
        delta=0.0            
        ...
        ecrire_un_message("tache_capteur_temperature", "Chauffage allumé" if chauffage_is_on else "Chauffage eteint")      
        verrou.release()
        
def tache_screen(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou) :
    while True:
        verrou.acquire()
        ecrire_donnees_temp(Cste_Consigne_Temperature, val_temperature, verrou)
        ecrire_donnees_pression(Cste_Consigne_Pression, val_pression, verrou)
        ecrire_etats_T_P_et_rel_TP(chauffage_is_on, pompe_is_on, verrou) 
        verrou.release()

def tache_controleur_central(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou):  
    while True :
        time.sleep(1)
        verrou.acquire()
        ...
        if (val_pression < min_pression_possible) :
            ...
            pompe_is_on=True
        verrou.release()
        ecrire_un_message("tache_controleur_central : Chauffage" , " --> j'allume" if pompe_is_on else "--> j'teints")
        ecrire_un_message("tache_controleur_central : Pompe", " --> j'allume" if pompe_is_on else "--> j'teints")
...
if __name__ == "__main__" :
    ...
    verrou=mp.Lock()
    chauffage_is_on= mp.Value(ctypes.c_bool,chauffage_init_on)
    pompe_is_on = mp.Value(ctypes.c_bool,Pompe_init_on)
    val_temperature = mp.Value(ctypes.c_float, Temp_init)
    val_pression = mp.Value(ctypes.c_float, Pression_init)
    ...
    mes_process[0] = mp.Process(target=tache_capteur_temperature,args=(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou))
    mes_process[1] = mp.Process(target=tache_capteur_pression,args=(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou))
    mes_process[2] = mp.Process(target=tache_controleur_central,args=(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou))
    mes_process[3] = mp.Process(target=tache_screen,args=(chauffage_is_on, val_temperature, pompe_is_on, val_pression,verrou))
    for i in range(nb_process) : mes_process[i].start()
    try : time.sleep(1) 
    except : 
